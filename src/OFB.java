...
private static long cipher;
	...
	public static void main(String[] args) {
	    long originalKey = (long)UUID.randomUUID().getLeastSignificantBits();
		long pText= (long)UUID.randomUUID().getLeastSignificantBits();
		
		init();
		
        double iter = Math.pow(2, 20) / 64;
        double Ones = 0;
        double Zeros = 0;
        
		for(int i = 0;i < iter;i++,RH = 0, LH = 0, cipher = 0){
		  DES(originalKey, pText);
		  int roundOnes = Long.bitCount(cipher);
		  Ones += roundOnes;
		  Zeros += 64 - roundOnes;
		  pText = cipher;
		}
        ...
        System.out.printf("%.0f",Zeros); System.out.println();
        System.out.print("Number of Ones:  ");
        System.out.printf("%.0f",Ones);
	}
	
	public static void DES(long key, long text){
		KeyGen(key);
        IP(text);
        Cycle();
        InverseIP();
